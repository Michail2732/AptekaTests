...
{
    public class DeleteBatchCommand : ConsoleCommand
    {
        private readonly IBatchesGateway _batchesGateway;

        public DeleteBatchCommand(IBatchesGateway batchesGateway): base(CommandBuildInfoRegistry.DeleteBatch)
        {
        ...
        public override async Task<ExecuteResult> ExecuteAsync(IDictionary<string, string> parameters)
        {
            try
            {
                ...
                await _batchesGateway.DeleteAsync(id);
